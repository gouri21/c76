...
export default class searchScreen extends React.Component {
  ...
  fetchtransaction = async()=>{
    ...
    if(textentered[0].toUpperCase==='B'){
      const transaction = await db.collection("transaction").where('bookid', '==', text).get().startAfter(this.state.lastvisibleTransaction).limit(15).get()
      ...
      const transaction = await db.collection("transaction").where('studentid', '==', text).get().startAfter(this.state.lastvisibleTransaction).limit(15).get()
      transaction.docs.map((doc)=>{
        this.setState({
          allTransactions:[...this.state.allTransactions, doc.data()],
          lastvisibleTransaction:doc
  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchBar}> <TextInput style={styles.bar}
